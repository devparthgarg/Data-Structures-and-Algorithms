...
public:
	...
	//array given,elemets in any order convert into minheap
	void buildHeap()
	{
		//start heapifying from lower levels
		//to higher levels and build heap
		//n: no of nodes
		//h: level at which we are counting nodes
		//max no. of nodes at height h: ceil(n/2^(h+1))
		//time: O(n)
        for(int i=(size-2)/2;i>=0;i--)
        {
        	//first calling for height 1 nodes then 
        	//height 2 nodes and so on
            minHeapify(i);
        }
    }
};
